...
const PaymentStatusBadge = ({ status }) => {
  ...
  return (
    <span className={`badge badge-sm capitalize ${styles[status] || 'badge-ghost'}`}>
      {status}
    </span>
  );
};
  ...
  return (
    <div>
      <div className="mb-8">
        ...
        <p className="text-text-muted mt-1">Track your reservation requests and payment status</p>
      </div>
      ...
      {bookings.length === 0 ? (
        <div className="bg-surface rounded-xl border border-border-subtle">
          <EmptyState
            ...
            action={
              <Link to="/properties" className="btn btn-primary-nestify">
                Explore Properties
              </Link>
            }
          />
