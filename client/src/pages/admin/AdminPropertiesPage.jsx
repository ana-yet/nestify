  ...
  return (
    <dialog open className="modal modal-open">
      <div className="modal-box">
        ...
        <form onSubmit={handleSubmit((data) => onConfirm(data.rejectionFeedback))}>
          <label className="form-label-nestify">Rejection Feedback (required)</label>
          <textarea
            rows={4}
          ...
          {errors.rejectionFeedback && (
            <p className="text-error text-sm mt-1">{errors.rejectionFeedback.message}</p>
          )}
...
const AdminPropertiesPage = () => {
  ...
  return (
    <div>
      ...
      <p className="text-text-muted mb-8">Moderate listings — approve, reject, or remove</p>
      ...
      <div className="bg-surface rounded-xl border border-border-subtle overflow-x-auto shadow-ambient">
        <table className="table w-full">
          ...
          <tbody>
            {properties.map((property) => {
              ...
              return (
                <tr key={property._id}>
                  <td className="font-medium max-w-[180px] truncate">{property.title}</td>
                  <td className="text-sm">{property.ownerInfo?.name}</td>
                  <td className="text-sm text-text-muted">{formatLocation(property.location)}</td>
                  <td>${amount}{suffix}</td>
                  <td><StatusBadge status={property.status} /></td>
                  <td>
                    <div className="flex flex-wrap gap-1">
                      <Link
                        to={`/dashboard/admin/edit-property/${property._id}`}
        ...
        {properties.length === 0 && (
          <p className="text-center text-text-muted py-12">No properties in the system.</p>
        )}
      </div>
