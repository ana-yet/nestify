...
import { fetchOwnerProperties, deleteProperty } from '../../api/properties.api.js';
    ...
    <div>
      <div className="flex justify-between items-center mb-8">
        <div>
          ...
          <p className="text-text-muted mt-1">Manage your listings and track approval status</p>
        </div>
        <Link to="/dashboard/owner/add-property" className="btn btn-primary-nestify btn-sm">
          Add Property
        </Link>
      </div>
      ...
      <div className="bg-surface rounded-xl border border-border-subtle overflow-hidden shadow-ambient">
        <div className="overflow-x-auto">
          <table className="table w-full">
            ...
            <tbody>
              {properties.map((property) => {
                ...
                return (
                  <tr key={property._id} className="hover:bg-surface-container-low/50">
                    <td className="font-medium">{property.title}</td>
                    <td className="text-text-muted text-sm">{formatLocation(property.location)}</td>
                    <td>
                      ${amount}
                      {suffix}
                    </td>
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        {property.status === 'rejected' && property.rejectionFeedback && (
                          <button
                            ...
                            onClick={() => setRejectionView(property.rejectionFeedback)}
                          >
                    ...
                    <td>
                      <div className="flex gap-2">
                        <Link
                          to={`/dashboard/owner/edit-property/${property._id}`}
          ...
          {properties.length === 0 && (
            <p className="text-center text-text-muted py-12">No properties yet. Add your first listing!</p>
          )}
        </div>
      </div>

      {rejectionView && (
        <RejectionModal feedback={rejectionView} onClose={() => setRejectionView(null)} />
      )}
    </div>
  );
};
