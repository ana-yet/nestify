...
const TENANT_LINKS = [
  ...
  { to: '/dashboard/tenant/bookings', label: 'My Bookings', icon: 'calendar_month' },
...
const OWNER_LINKS = [
  ...
  { to: '/dashboard/owner/bookings', label: 'Owner Bookings', icon: 'calendar_month' },
  { to: '/dashboard/owner/add-property', label: 'Add Property', icon: 'add_home' },
...
const ADMIN_LINKS = [
  ...
  { to: '/dashboard/admin/properties', label: 'All Properties', icon: 'domain' },
  { to: '/dashboard/admin/bookings', label: 'All Bookings', icon: 'calendar_month' },
  { to: '/dashboard/admin/transactions', label: 'Transactions', icon: 'payments' },
...
const MobileDashboardNav = ({ role }) => {
  ...
  const linkClass = ({ isActive }) =>
    `flex items-center gap-3 px-4 py-3 rounded-lg text-sm font-medium ${
      isActive ? 'bg-primary/10 text-primary font-bold' : 'text-on-surface-variant'
    }`;
        ...
        <div className="fixed inset-0 z-50 md:hidden">
          ...
          <aside className="absolute left-0 top-0 h-full w-72 bg-surface border-r border-border-subtle shadow-xl flex flex-col p-4">
            ...
            <div className="pt-4 border-t border-border-subtle space-y-1">
              <Link to="/" className="flex items-center gap-3 px-4 py-2 text-sm" onClick={() => setOpen(false)}>
