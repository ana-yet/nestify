...
const verifyFirebaseToken = async (idToken) => {
  ...
  if (!auth) throw Object.assign(new Error('Firebase not configured'), { statusCode: 500 });
...
export const login = async (req, res, next) => {
  try {
    ...
    else if (!isGoogle && user.authProvider === 'google') user.authProvider = 'both';
...
export const googleLogin = async (req, res, next) => {
  try {
    ...
    if (!email) return res.status(400).json({ success: false, message: 'Google account must have email' });
