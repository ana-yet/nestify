...
export const getBookingById = catchAsync(async (req, res) => {
  ...
  const isTenant = booking.tenantId._id
    ? booking.tenantId._id.toString() === req.user.userId
    : booking.tenantId.toString() === req.user.userId;
  const isOwner = booking.ownerId.toString() === req.user.userId;
...
export const rejectBooking = catchAsync(async (req, res) => {
  ...
  if (booking.paymentStatus === 'paid') {
    throw new ApiError(400, 'Cannot reject a booking that has already been paid');
  }
