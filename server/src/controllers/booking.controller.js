...
export const createBooking = async (req, res, next) => {
  try {
    ...
    if (property.ownerId.toString() === req.user.userId) {
      return res
        .status(400)
        .json({ success: false, message: "Cannot book your own property" });
    }
...
export const approveBooking = async (req, res, next) => {
  try {
    ...
    if (booking.paymentStatus !== "paid") {
      return res.status(400).json({
        success: false,
        message: "Booking must be paid before approval",
      });
    }
    ...
    if (booking.bookingStatus === "rejected") {
      return res
        .status(400)
        .json({ success: false, message: "Cannot approve a rejected booking" });
    }
    ...
    res
      .status(200)
      .json({ success: true, message: "Booking approved", data: { booking } });
  } catch (error) {
    next(error);
  }
};
...
export const rejectBooking = async (req, res, next) => {
  try {
    ...
    if (booking.bookingStatus === "approved") {
      return res
        .status(400)
        .json({ success: false, message: "Cannot reject an approved booking" });
    }
    ...
    res
      .status(200)
      .json({ success: true, message: "Booking rejected", data: { booking } });
  } catch (error) {
    next(error);
  }
};
